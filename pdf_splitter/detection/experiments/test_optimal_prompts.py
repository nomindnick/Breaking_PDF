...
class OptimalPromptTester:
    ...
    def test_optimal_prompts(
        ...
        for model in models:
            ...
            optimal_prompt = f"{model_family}_optimal" if model_family in ["phi", "gemma"] else None
    ...
    def _test_single_prompt(
        ...
        for difficulty in difficulty_levels:
            diff_cases = [
                c for c in self.synthetic_tester.test_cases 
            ...
            for case in diff_cases:
                ...
                try:
                    ...
                    if post_process:
                        prediction, confidence = self.synthetic_tester.post_process_response(
            ...
            diff_results["accuracy"] = (
                ...
                if diff_results["total"] > 0 else 0.0
            )
        ...
        if results["overall"]["total"] > 0:
            ...
            true_positives = sum(
                1 for d in results["by_difficulty"].values()
                for detail in d["details"]
                if detail.get("expected") == "Different" and detail.get("predicted") == "Different"
            )
            false_positives = sum(
                1 for d in results["by_difficulty"].values()
                for detail in d["details"]
                if detail.get("expected") == "Same" and detail.get("predicted") == "Different"
            )
            false_negatives = sum(
                1 for d in results["by_difficulty"].values()
                for detail in d["details"]
                if detail.get("expected") == "Different" and detail.get("predicted") == "Same"
            )
            
            if true_positives + false_positives > 0:
                results["overall"]["precision"] = true_positives / (true_positives + false_positives)
            if true_positives + false_negatives > 0:
                results["overall"]["recall"] = true_positives / (true_positives + false_negatives)
            
            if results["overall"]["precision"] + results["overall"]["recall"] > 0:
                results["overall"]["f1_score"] = (
                    2 * results["overall"]["precision"] * results["overall"]["recall"] /
        ...
        for name, results in model_results["optimal"].items():
            if isinstance(results, dict) and "error" not in results:
                ...
                improvements["optimal_improvement"] = (
                    ...
                    if best_baseline_f1 > 0 else 0.0
                )
                
                # Latency improvement
                baseline_time = baseline_results.get("timing", {}).get("avg_per_case", 0.0)
    ...
    def _generate_summary(self, results: Dict) -> Dict:
        ...
        for model, model_results in results["models"].items():
            for category in ["baseline", "optimal", "cod"]:
                for prompt_name, prompt_results in model_results.get(category, {}).items():
                    if isinstance(prompt_results, dict) and "error" not in prompt_results:
        ...
        if optimal_improvements:
            summary["average_improvements"]["optimal_over_baseline"] = (
                sum(optimal_improvements) / len(optimal_improvements)
            )
        if cod_improvements:
            summary["average_improvements"]["cod_over_baseline"] = (
                sum(cod_improvements) / len(cod_improvements)
            )
    ...
    def print_summary(self, results: Dict):
        ...
        for model, rec in summary.get("model_recommendations", {}).items():
            print(f"  {model}: {rec.get('best_prompt')} (F1: {rec.get('f1_score', 0.0):.3f})")
        ...
        for model, model_results in results.get("models", {}).items():
            ...
            if improvements:
                print(f"\n  {model}:")
                print(f"    Baseline: {improvements.get('best_baseline')} (F1: {improvements.get('baseline_f1', 0.0):.3f})")
                if "optimal_f1" in improvements:
                    print(f"    Optimal: F1: {improvements.get('optimal_f1', 0.0):.3f} ({improvements.get('optimal_improvement', 0.0):+.1f}%)")
                if "cod_f1" in improvements:
                    print(f"    Best CoD: {improvements.get('best_cod')} F1: {improvements.get('cod_f1', 0.0):.3f} ({improvements.get('cod_improvement', 0.0):+.1f}%)")
                if "latency_improvement" in improvements:
                    print(f"    Latency: {improvements.get('latency_improvement', 0.0):+.1f}% faster")
