...
# Load the results
results_file = Path(
    "pdf_splitter/detection/experiments/results/optimal_prompt_test_20250704_173553.json"
)
...
for prompt_name, perf_data in prompt_performance.items():
    if perf_data["models"]:
        avg_f1 = sum(m["f1_score"] for m in perf_data["models"].values()) / len(
            perf_data["models"]
        )
        avg_acc = sum(m["accuracy"] for m in perf_data["models"].values()) / len(
            perf_data["models"]
        )
...
sorted_prompts = sorted(
    prompt_performance.items(), key=lambda x: x[1]["avg_f1"], reverse=True
)
for prompt_name, perf_data in sorted_prompts:
    models_tested = len(perf_data["models"])
    print(
        f"{prompt_name:<30} {perf_data['avg_f1']:<10.3f} {perf_data['avg_accuracy']:<10.2%} {models_tested}"
    )
            ...
            if "by_difficulty" in prompt_data:
                for diff_level, diff_data in prompt_data["by_difficulty"].items():
                    if prompt_name != "A1_asymmetric":  # Skip the broken prompt
                        ...
                        difficulty_stats[int(diff_level)]["correct"] += diff_data[
                            "correct"
                        ]
...
for model, model_data in data["models"].items():
    for category in ["baseline", "cod"]:
        for prompt_name, prompt_data in model_data.get(category, {}).items():
            if (
                "overall" in prompt_data
                and prompt_data["overall"].get("f1_score", 0) > 0
            ):
...
for combo in top_combinations[:10]:
    print(
        f"{combo['model']:<20} {combo['prompt']:<25} {combo['f1']:<10.3f} {combo['accuracy']:<10.2%} {combo['recall']:.2%}"
    )
...
for prompt_name, counts in prediction_counts.items():
    ...
    if total > 0:
        print(
            f"{prompt_name:<30} {counts['Different']:<12} {counts['Same']:<12} {counts['Unknown']}"
        )
...
print(
    "- Optimal prompts (phi4_optimal, gemma3_optimal) were not tested due to naming mismatch"
)
