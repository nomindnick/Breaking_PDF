...
class TestVisualDetectorIntegration:
    ...
    def create_test_image(self, color: tuple = (255, 255, 255), text: str = "") -> bytes:
    ...
    def create_pages_with_images(self, num_pages: int = 5) -> list[ProcessedPage]:
        ...
        for i in range(num_pages):
            ...
            if i < 2:
                ...
                image_bytes = self.create_test_image((255, 255, 255), f"DOCUMENT 1 - HEADER\n\n\nContent of page {i+1}")
            elif i < 4:
                # Document 2: Dark gray pages with different layout
                image_bytes = self.create_test_image((100, 100, 100), f"===== DOCUMENT 2 =====\n\n\n\n\nPage {i+1} content here")
            else:
    ...
    def test_visual_detector_with_signal_combiner(self):
        ...
        for boundary in boundaries:
            assert boundary.detector_type in [DetectorType.VISUAL, DetectorType.COMBINED]
    ...
    def test_visual_detector_fallback_to_pdf(self):
        ...
        mock_pdf_handler.render_page.return_value = np.zeros((600, 800, 3), dtype=np.uint8)
    ...
    def test_visual_detector_error_handling(self):
        ...
        for i in range(3):
            page = ProcessedPage(
                ...
                text=f"Page {i + 1} content",
                ocr_confidence=0.95,
                rendered_image=b"invalid image data" if i == 1 else self.create_test_image()
            )
            pages.append(page)
    ...
    def test_mixed_rendered_and_pdf_pages(self):
        ...
        mock_pdf_handler = Mock()
        mock_pdf_handler.render_page.return_value = np.ones((600, 800, 3), dtype=np.uint8) * 255
